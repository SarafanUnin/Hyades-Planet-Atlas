...
def build_specific_energy(
    ...
    # The atlas models the accessible surface. Negative source elevations
    # represent submerged terrain and are therefore evaluated at sea level.
    effective_elevation_m = np.maximum(
        elevation,
        0.0,
    )
...
def main() -> None:
    ...
    # Preserve whether each source cell was at or below sea level before
    # clamping. This treats exact-zero cells as ocean as well, which is useful
    # for synthetic planetary DEMs whose oceans are encoded as 0 m.
    ocean_mask = (
        elevation_m <= 0.0
    )

    # Store only a positive, offset energy magnitude, then use the float sign
    # as an ocean flag:
    #
    #     positive value = above-sea-level destination
    #     negative value = ocean-surface destination
    #
    # The offset is placed just below the physical minimum, guaranteeing that
    # every magnitude is at least 1 J/kg. Taking abs() therefore recovers the
    # energy scalar, and the common offset still cancels from differences.
    storage_floor_margin_j_per_kg = 1.0

    energy_offset_j_per_kg = float(
        total_energy.min() -
        storage_floor_margin_j_per_kg
    )

    stored_energy_magnitude = (
        total_energy -
        energy_offset_j_per_kg
    )

    if np.any(stored_energy_magnitude <= 0.0):
        raise RuntimeError(
            "Stored energy magnitudes must be strictly positive."
        )

    encoded_energy = np.where(
        ocean_mask,
        -stored_energy_magnitude,
        stored_energy_magnitude,
    ).astype("<f4")
    ...
    output_metadata = {
        ...
        "stored_value_definition": (
            "sign * (physical_energy_j_per_kg - energy_offset_j_per_kg)"
        ),
        "stored_value_sign_encoding": {
            "positive": "source elevation > 0 m",
            "negative": "source elevation <= 0 m; energy evaluated at 0 m",
            "decode_energy": "abs(stored_value)",
            "render_land_only": "discard stored_value < 0",
        },
        "surface_elevation_policy": {
            "energy_calculation": "negative elevations clamped to 0 m",
            "ocean_flag": "original elevation <= 0 m",
            "sea_level_m": 0.0,
        },
        "storage_floor_margin_j_per_kg": (
            storage_floor_margin_j_per_kg
        ...
        "minimum_encoded_value": float(
            stored_energy.min()
        ),
        "maximum_encoded_value": float(
            stored_energy.max()
        ),
        "minimum_decoded_stored_j_per_kg": float(
            stored_energy_magnitude.min()
        ),
        "maximum_decoded_stored_j_per_kg": float(
            stored_energy_magnitude.max()
        ),
        "ocean_cell_count": int(
            np.count_nonzero(ocean_mask)
        ),
        "above_sea_level_cell_count": int(
            np.count_nonzero(~ocean_mask)
        ),
        ...
        "delta_sign_convention": {
            "released_energy_j_per_kg": (
                "abs(origin_stored_energy) - "
                "abs(destination_stored_energy)"
            ),
    ...
    print(
        ...
        f"{stored_energy.max():,.3f}"
    )
    print(
        "Ocean-flagged cells: "
        f"{np.count_nonzero(ocean_mask):,} / {ocean_mask.size:,}"
    )
